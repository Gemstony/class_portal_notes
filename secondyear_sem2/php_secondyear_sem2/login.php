...
$stmt = $conn->prepare("SELECT * FROM users WHERE username = ? AND password =? AND  (year = 'SECOND' OR role = 'SUPER'); ");
...
if ($stmt->num_rows > 0) {
    $user_name;
    $sql = "SELECT user_id FROM users WHERE username = '$user_name' AND (year = 'SECOND' OR role = 'SUPER');";
